...
public class ObservableProgress : ObservableObject {
    ...
    public func update(current: Int, total: Int? = nil, title: String? = nil, progressBarTitleStyle: ProgressBarTitleStyle? = nil) {
        DispatchQueue.main.async {
            self.nextUpdate = Update(current: current, total: total ?? self.total, title: title ?? self.title, progressBarTitleStyle: progressBarTitleStyle ?? self.progressBarTitleStyle)
            self.debouncer?.call()
        }
    }
